...
class Gutenberg {
	...
	private function __construct() {
		...
		if (file_exists($this->blocks_paths)) {
			$theme_blocks = array_diff(scandir($this->blocks_paths), apply_filters('fff/gutenberg/blocks_path/excluding_folders', ['.', '..', '.DS_Store']));
            ...
            foreach ($theme_blocks as $theme_block) {
                ...
                if (!empty($init_path)) {
                    $opt = include $init_path;

                    if (empty($opt) || !is_array($opt)) continue;
    ...
    public static function locateFile($file_name, $template_paths = [], $return_url=false) {
        ...
        foreach ($template_paths as $path) {
            if (file_exists($path . $file_name)) {
	            $url_dir = str_replace(str_replace(DIRECTORY_SEPARATOR, '/', ABSPATH), '/', str_replace(DIRECTORY_SEPARATOR,'/', $path));
