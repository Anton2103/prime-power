...
class Helpers
{
    ...
    public static function sanitizeTextField($txt){
        return trim(sanitize_text_field( html_entity_decode($txt)));
    }

    public static function restAddSanitizeTextToResponseACF(&$response, $fields, $element, $suf = '_text') {
        if (empty($response)) return;

        foreach ($response as $key=>$response_element) {
            if (array_key_exists('acf', $response_element) && array_key_exists($fields, $response_element['acf']) && is_array($response_element['acf'][$fields] )) {
                foreach ($response_element['acf'][$fields] as $sanitize_key => $sanitize_item) {
                    $response[$key]['acf'][$fields][$sanitize_key][$element . $suf] = self::sanitizeTextField($sanitize_item[$element]);
                }
            }
        }
    }
    ...
    public static function is_realy_woocommerce_page () {
        ...
        foreach ( $woocommerce_keys as $wc_page_id ) {
            if ( $current_page_id== get_option ( $wc_page_id , 0 ) ) {
